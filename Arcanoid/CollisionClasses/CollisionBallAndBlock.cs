...
{
    public class CollisionBallAndBlock : ICollisionBall
    {
        ...
        {
            _intersectRectangle = Rectangle.Intersect(new Rectangle(ball.Position, ball.Size), new Rectangle(gameObject.Location, gameObject.Size));
            ...
            if (pointCenterBall.Y > gameObject.Location.Y)
            {
                ball.SpeedVector = new Point(ball.SpeedVector.X, -1 * ball.SpeedVector.Y);
            }
